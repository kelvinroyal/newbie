...
if(isset($_POST['submit'])){
    ...
    if(isset($ten) && isset($dien_thoai) && isset($binh_luan)){
        $sql="INSERT INTO blsanpham(ten,dien_thoai,binh_luan,ngay_gio,id_sp,check_bl) VALUES('$ten',"
                . "'$dien_thoai','$binh_luan','$ngay_gio','$id_sp','Y')";
        $query= mysqli_query($conn, $sql);
...
$sqlBl="SELECT * FROM blsanpham WHERE check_bl='Y' AND id_sp=$id_sp ORDER BY id_bl ASC LIMIT $perRow,$rowsPerPage";
$queryBl= mysqli_query($conn, $sqlBl);

//tong so ban ghi (tong so binh luan cua san pham do)
$totalRows= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM blsanpham WHERE check_bl='Y' AND id_sp=$id_sp"));
